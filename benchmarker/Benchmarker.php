...
class Benchmarker {
    ...
    public function run() {
        ...
        foreach ($config['benchmarker.processes'] as $name => $count) {
            for ($i = 0; $i < $count; $i++) {
                $fork_process_mode[] = $name;
            }
        }
        ...
        if (!empty($config['global']['initializer'])) {
            call_user_func([$this, $config['global']['initializer']]);
        }
    ...
    protected function notify_score($identifier, $score) {
        ...
        $fp = fopen(self::SHARED_TMP_MAP, 'c+');
        if (flock($fp, LOCK_EX)) {
            $map = json_decode(fread($fp, filesize(self::SHARED_TMP_MAP) + 1), true);
            $map = ($map !== null) ? $map : [];
    ...
    protected function figure_scores($pids) {
        ...
        foreach ($pids as $pid) {
            $key = isset($map["score_{$pid}_type"]) ? $map["score_{$pid}_type"] : null;
            if ($key !== null) {
